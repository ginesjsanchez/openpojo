...
public class ExceptionsTest {
	...
	public void checkCount() {
		Affirm.affirmEquals(
				String.format("Classes added/removed that implement Throwable found[%s]?", pojoExceptionClasses),
	...
	public void ensureConstructorsPrivate(final List<PojoMethod> constructors) {
		...
		for (final PojoMethod constructor : constructors) {
			Affirm.affirmTrue(String.format("Constructor must be private [%s]!!", constructor),
		...
		for (final PojoMethod getInstance : pojoExceptionClass.getPojoMethods()) {
			if (getInstance.getName().equals("getInstance")) {
				...
				if (getInstance.getParameterTypes().length == 2) {
					...
					Affirm.affirmEquals(
							String.format("Message changed in Exception[%s] using getInstance(String, Throwable)?!",
									pojoExceptionClass),
							someMessage, instance.getMessage());
					Affirm.affirmEquals(
							String.format("Cause changed in Exception[%s] using getInstance(String, Throwable)?!",
									pojoExceptionClass),
							cause, instance.getCause());
				}
				getInstanceCount++;
			}
		}
