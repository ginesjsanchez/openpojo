...
public class GenericArrayTypeResolverTest {
	...
	public void end2endTest() {
		AClassWithGenericArrayType aClassWithGenericArray = RandomFactory
				.getRandomValue(AClassWithGenericArrayType.class);
		Assertions.assertNotNull(aClassWithGenericArray);
		Assertions.assertTrue(aClassWithGenericArray.getMySetOfArrayElements().size() > 0);
		Object[] objects = aClassWithGenericArray.getMySetOfArrayElements().toArray();
		Assertions.assertTrue(objects.length > 0);
		Assertions.assertTrue(objects[0] instanceof ArrayElementsClass);
	}
