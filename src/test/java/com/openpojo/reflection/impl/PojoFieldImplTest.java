...
public class PojoFieldImplTest {
	...
	/**
	 * Test method for:
	 * {@link com.openpojo.reflection.impl.PojoFieldImpl#get(java.lang.Object)}.
		...
		for (PojoField pojoField : pojoClass.getPojoFields()) {
			if (!pojoField.isFinal() && !pojoField.isPrimitive()) {
				...
				Affirm.affirmEquals(String.format(
						...
						pojoField.get(pojoClassInstance), randomValue, pojoField), randomValue,
						pojoField.get(pojoClassInstance));
			}
		}
	}
	...
	public void shouldGetParameterizedType() {
		...
		for (PojoField pojoField : pojoClass.getPojoFields()) {
			if (pojoField.getName().equals("parameterizedChildren")) {
				...
				Affirm.affirmTrue("Wrong Parameterization!?",
						pojoField.getParameterTypes().contains(ClassWithGenericTypes.class));
				affirmChecks++;
			}
			...
			if (pojoField.getName().equals("nonparameterizedList")
				...
				Affirm.affirmEquals(
						"Returned non-empty list for nonParameterized type!? [" + pojoField.getParameterTypes() + "]",
						0, pojoField.getParameterTypes().size());
				affirmChecks++;
			}
			...
			if (pojoField.getName().equals("parameterizedMap")) {
				...
				Affirm.affirmTrue(String.format("Type not found [%s]", String.class),
						pojoField.getParameterTypes().contains(String.class));
				Affirm.affirmTrue(String.format("Type not found [%s]", Integer.class),
						pojoField.getParameterTypes().contains(Integer.class));
				affirmChecks++;
			}
		}
	...
	public void multipleAnnotationsShouldBeReturned() {
		...
		for (PojoField pojoField : allFields) {
			if (pojoField.getName().equals("multipleAnnotationField")) {
				Affirm.affirmEquals(String.format("Annotations added/removed from field=[%s]", pojoField), 2,
						pojoField.getAnnotations().size());
				...
				for (Annotation annotation : pojoField.getAnnotations()) {
					Affirm.affirmTrue(
							String.format("Expected annotations [%s] not found, instead found [%s]",
	...
	/**
	 * Test method for
	 * {@link com.openpojo.reflection.impl.PojoFieldImpl#isPrimitive()}.
	 */
	@Test
	public void testIsPrimitive() {
		for (PojoField pojoField : pojoClass.getPojoFields()) {
			if (pojoField.getName().startsWith("primitive")) {
				Affirm.affirmTrue(
						String.format("isPrimitive() check on primitive field=[%s] returned false!!", pojoField),
						pojoField.isPrimitive());
			}
		}
	}
	...
	/**
	 * Test method for
	 * {@link com.openpojo.reflection.impl.PojoFieldImpl#isStatic()}.
	 */
	@Test
	...
	public void testIsPrivate() {
		...
		Affirm.affirmFalse("isPackagePrivate() check on field=[" + pojoField + "] returned true!!",
				pojoField.isPackagePrivate());
	}
	...
	public void isPackagePrivate() {
		...
		PojoField pojoField = getFieldStartingWith(prefix);
		Affirm.affirmNotNull("Field not found [" + prefix + "]", pojoField);
		Affirm.affirmTrue("isPackagePrivate() check on field=[" + pojoField + "] returned false!!",
				pojoField.isPackagePrivate());
...
//	}
	...
	public void canGetEnclosingClass() {
		...
		for (PojoField field : pojoClassWithFields.getPojoFields()) {
			Affirm.affirmEquals("Failed to get PojoClass from field [" + field + "]", pojoClassWithFields,
					field.getDeclaringPojoClass());
		}
	}
}
