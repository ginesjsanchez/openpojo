...
public class PojoClassFactoryTest {
	...
	public void testGetPojoClass() {
		...
		Affirm.affirmNotNull(String.format("PojoClassFactory failed to create PojoClass for [%s]", this.getClass()),
				pojoClass);
		Affirm.affirmEquals(
				String.format("PojoClassFactory returned invalid meta-definition PojoClass for [%s]", this.getClass()),
	...
	public void testGetPojoClasses() {
		...
		for (Class<?> clazz : DUMMY_PACKAGE_CLASSES) {
			Affirm.affirmTrue(
					String.format("Unexpected class=[%s] retrieved from package=[%s], ", clazz, DUMMY_PACKAGE),
	...
	public void testGetFilteredPojoClasses() {
		...
		Affirm.affirmTrue(String.format(
				...
				DUMMY_PACKAGE, loggingPojoClassFilter
						.getPojoClassCallLogs(),
				loggingPojoClassFilter.getPojoClassCallLogs().size()),
				2 <= loggingPojoClassFilter
		...
		Affirm.affirmEquals(
				String.format("Wrong number of classes retrieved!! Classes added/removed from package=[%s]?",
						DUMMY_PACKAGE),
				2, pojoClasses.size());
	}
	...
	public void testEnumerateClassesByExtendingType() {
		List<PojoClass> pojoClasses = PojoClassFactory.enumerateClassesByExtendingType(DUMMY_PACKAGE, Persistable.class,
				null);
	...
	public void shouldGenerateAppropriateLinkErrorForInvalidASMGeneratedClasses() throws Exception {
		...
		final String className = this.getClass().getPackage().getName() + ".AClassWithBadMethod"
				+ GENERATED_CLASS_POSTFIX;

		final String classNameAsPath = className.replace(Java.PACKAGE_DELIMITER, Java.PATH_DELIMITER);
		final Class<?> clazz = simpleClassLoader.loadThisClass(AClassWithBadMethodDump.dump(classNameAsPath),
				className);
