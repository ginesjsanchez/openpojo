...
import com.openpojo.reflection.impl.sample.classes.AClassExtendingAnInterface;
import com.openpojo.reflection.impl.sample.classes.AClassWithAnnotatedFields;
import com.openpojo.reflection.impl.sample.classes.AClassWithAnnotatedMethods;
import com.openpojo.reflection.impl.sample.classes.AClassWithAnnotations;
import com.openpojo.reflection.impl.sample.classes.AClassWithEquality;
import com.openpojo.reflection.impl.sample.classes.AClassWithExceptionalConstructors;
import com.openpojo.reflection.impl.sample.classes.AClassWithInterfaces;
...
import com.openpojo.reflection.impl.sample.classes.AClassWithSixMethods;
import com.openpojo.reflection.impl.sample.classes.AClassWithSythetics;
import com.openpojo.reflection.impl.sample.classes.AClassWithTwoChildClassesOneStaticAndOneNot;
import com.openpojo.reflection.impl.sample.classes.AClassWithoutInterfaces;
import com.openpojo.reflection.impl.sample.classes.AClassWithoutMethods;
import com.openpojo.reflection.impl.sample.classes.AFinalClass;
import com.openpojo.reflection.impl.sample.classes.ANonFinalClass;
import com.openpojo.reflection.impl.sample.classes.AnAbstractClass;
import com.openpojo.reflection.impl.sample.classes.AnInterfaceClass;
import com.openpojo.reflection.impl.sample.classes.FirstInterfaceForAClassWithInterfaces;
import com.openpojo.reflection.impl.sample.classes.MultiplePublicAndPrivateWithManyParamsConstructor;
import com.openpojo.reflection.impl.sample.classes.NoDeclaredConstructor;
import com.openpojo.reflection.impl.sample.classes.OnePrivateNoParamsConstructor;
import com.openpojo.reflection.impl.sample.classes.OnePublicNoParamConstructor;
		...
		for (final PojoClass pojoClass : PojoClassFactory.getPojoClassesRecursively(SAMPLE_CLASSES_PKG, null)) {
			...
			Affirm.affirmTrue(
					String.format(message, actualClass.getName() + ".isAbstract()",
							...
							pojoClass.isAbstract()),
					pojoClass.isAbstract() == (Modifier.isAbstract(actualClass.getModifiers())
							&& !Modifier.isInterface(actualClass.getModifiers())));

			final boolean expectedValue = !(Modifier.isAbstract(actualClass.getModifiers()) || actualClass.isInterface()
					|| actualClass.isEnum());
			final boolean actualValue = pojoClass.isConcrete();
			Affirm.affirmTrue(
					String.format(message, actualClass.getName() + ".isConcrete()", expectedValue, actualValue),
...
//	}
	...
	public void testGetPojoMethods() {
		...
		Affirm.affirmEquals(String.format("Methods added/removed from class=[%s]", pojoClass.getName()), 0 + 1 /*
																												 * constructor
																												 */,
				pojoClass.getPojoMethods().size());
	}
	...
	public void testExtendz() {
		...
		Affirm.affirmTrue(
				String.format("Failed to validate Class=[%s] extending an interface=[%s]" + " for PojoClass "
	...
	public void shouldCreateInstanceUsingDeclaredPublicConstructor() {
		...
		Affirm.affirmNotNull(
				String.format("Failed to create a new instance using publicly declared constructor for " + "class=[%s]",
						pojoClass),
				instance);
	}
	...
	public void shouldCreateInstanceUsingImplicitConstructor() {
		...
		Affirm.affirmNotNull(String.format(
				"Failed to create a new instance using compiler auto-generated constructor for " + "class=[%s]",
				pojoClass), instance);
	}
	...
	public void testIsNestedClass() {
		...
		Affirm.affirmTrue(
				String.format("Nested class=[%s] returned false for isNestedClass on PojoClass implementation=[%s]",
						nestedClass, pojoClass),
				pojoClass.isNestedClass());
	...
	public void testCopy() {
		...
		Affirm.affirmTrue(String.format(
				"Class=[%s] copied to=[%s] and still equals returned false using PojoClass" + " "
						+ "implementation=[%s]!!",
				BusinessIdentity.toString(first), BusinessIdentity.toString(second), pojoClass), first.equals(second));
	}
	...
	public void testGetClazz() {
		...
		Affirm.affirmTrue(
				String.format("PojoClass parsing for [%s] returned different class=[%s] in getClazz() call" + " for "
	...
	public void testIsStatic() {
		PojoClass pojoClass = getPojoClassImplForClass(
				AClassWithTwoChildClassesOneStaticAndOneNot.APublicNonStaticClass.class);
	...
	public void testGetPath() {
		...
		String thisClassEndingPath = this.getClass().getName().replace(Java.PACKAGE_DELIMITER, Java.PATH_DELIMITER)
				+ Java.CLASS_EXTENSION;
		Affirm.affirmTrue("Should end with this class's package path [" + sourcePath + "]",
				sourcePath.endsWith(thisClassEndingPath));
	}
	...
	public void isPublicClass() {
		...
		Affirm.affirmFalse("isPackagePrivate() check on class=[" + pojoclass + "] returned true!!",
				pojoclass.isPackagePrivate());
	}
	...
	public void isPackagePrivateClass() {
		...
		Affirm.affirmTrue("isPackagePrivate() check on class=[" + pojoclass + "] returned false!!",
				pojoclass.isPackagePrivate());
		Affirm.affirmFalse("isPrivate() check on class=[" + pojoclass + "] returned true!!", pojoclass.isPrivate());
		Affirm.affirmFalse("isProtected() check on class=[" + pojoclass + "] returned true!!", pojoclass.isProtected());
		Affirm.affirmFalse("isPublic() check on class=[" + pojoclass + "] returned true!!", pojoclass.isPublic());
