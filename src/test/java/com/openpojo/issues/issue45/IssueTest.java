...
public class IssueTest {
	...
	public void ensureAClassWithGenericCollectionPojo_HasNotBeenModified() {
		//// Assertions.//assertThat("Fields added / removed to sample class?",
		//// aClassWithGenericCollectionPojo.getPojoFields().size(), is(1));
		PojoField pojoField = aClassWithGenericCollectionPojo.getPojoFields().get(0);
		Assertions.assertEquals(List.class, pojoField.getType());
		//// Assertions.//assertThat("Field should have only one parameter",
		//// pojoField.getParameterTypes().size(), is(1));
		Assertions.assertEquals(SomeGeneric.class, pojoField.getParameterTypes().get(0));
	}

	@Test
	public void ensureAParameterizedClass_HasNotBeenModified() {
		//// Assertions.//assertThat(aParameterizedClass.getPojoFields().size(), is(1));
		//// Assertions.//assertThat(aParameterizedClass.getClazz().getTypeParameters().length,
		//// is(1));
		Assertions.assertTrue(
				TypeVariable.class.isAssignableFrom(aParameterizedClass.getClazz().getTypeParameters()[0].getClass()));
	}
	...
	public void testEndToEndCollections() {
		pojoValidator = ValidatorBuilder.create().with(new SetterMustExistRule()).with(new SetterTester()).build();
