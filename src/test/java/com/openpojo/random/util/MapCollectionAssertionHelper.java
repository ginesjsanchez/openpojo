...
public class MapCollectionAssertionHelper {
	...
	public static void assertStructure(Object input, LinkedList<Class<?>> queue) {
		...
		if (isAMap(input)) {
			...
			// Assertions.assertThat(inputMap.size(), is(greaterThan(0)));
	...
	private static void ensureThatAllKeysAndValuesAreSameClassType(Map<?, ?> inputMap) {
		...
		for (Map.Entry entry : inputMap.entrySet()) {
			Assertions.assertEquals(entry.getKey().getClass(), referenceEntry.getKey().getClass());
			Assertions.assertEquals(entry.getValue().getClass(), referenceEntry.getValue().getClass());
	...
	private static void ensureThatAllCollectionEntriesAreSameClassType(Collection collection) {
		...
		for (Object entry : collection) {
			Assertions.assertEquals(referenceEntry.getClass(), entry.getClass());
	...
	private static void assertTypeIsAssignable(Object input, Class<?> type) {
		Assertions.assertTrue(type.isAssignableFrom(input.getClass()));
	}
}
