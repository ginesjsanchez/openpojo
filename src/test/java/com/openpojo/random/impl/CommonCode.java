...
public class CommonCode {
	...
	public static void testDoGenerateForClass(final RandomGenerator randomGenerator, final Class<?> type) {
		Affirm.affirmTrue(
				String.format("Failed to get the requested type=[%s] from [%s] received=[%s] instead", type,
						randomGenerator, randomGenerator.doGenerate(type)),
				type.isInstance(randomGenerator.doGenerate(type)));
		...
		if (object.equals(anotherObject)) { // Just incase they are the same
			...
			// if Boolean, try for 20 times, since there is a 50% chance we land on the same
			// value.
	...
	public static void testGetType(final RandomGenerator randomGenerator, final Class<?> type,
			final int expectedCount) {
		Assertions.assertEquals(expectedCount, randomGenerator.getTypes().size());
		Assertions.assertTrue(randomGenerator.getTypes().contains(type));
	}
}
