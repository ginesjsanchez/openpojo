...
public class RandomInstanceFromInterfaceRandomGeneratorTest {
	...
	public void shouldReturnRandomNonNullValuesForInterfaceMethods() {
		...
		if (name.equals(otherName)) { // Just in case they are the same by chance.
			Affirm.affirmFalse(
					String.format("RandomProxyFactory=[%s] returned a non-Random Pojo Proxy",
	...
	public void shouldImplementAccuratetoStringAndhashCode() {
		...
		Affirm.affirmTrue(String.format("toString returned [%s] expected it to begin with [%s] and contain [@]",
				toString, "$Proxy"), toString.contains("$Proxy") && toString.contains("@"));

		Affirm.affirmTrue("toString() doesn't end with hashCode()",
				toString.endsWith(String.valueOf(aSimpleInterface.hashCode())));
	...
	public void shouldReturnProperGenericsValue() {
		final AnInterfaceWithGenericMethodReturnType anInterfaceWithGenericMethodReturnType = proxyGenerator
				.doGenerate(AnInterfaceWithGenericMethodReturnType.class);
