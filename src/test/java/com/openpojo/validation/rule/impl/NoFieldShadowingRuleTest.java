...
public class NoFieldShadowingRuleTest {
	...
	@BeforeEach
	public void setup() {
		validator = ValidatorBuilder.create().with(new NoFieldShadowingRule()).build();
	}
	...
	public void aClassWithSyntheticFieldShadowing() {
		PojoClass aClassWithSyntheticFieldShadowing = PojoClassFactory
				.getPojoClass(AClassWithSyntheticFieldShadowing.class);
...
//    Assertions.//assertThat(pojoFields.size(), is(1));
//    Assertions.//assertThat(pojoFields.get(0).getName(), is("aField"));
	}
...
//    Assertions.//assertThat(pojoFields.get(0).getName(), is("aField"));
	...
	public void aChildWithFieldShadowingShouldFail() {
		...
		try {
			...
			Assertions.fail("Expected [NoShadowAChildWithFieldShadowing.class] to fail NoFieldShadowRule but didn't");
		} catch (AssertionError ae) {
			Assertions.assertEquals("Field=[PojoFieldImpl "
					+ "[field=private java.lang.String com.openpojo.validation.rule.impl.sampleclasses.NoShadowAChildWithFieldShadowing.aField, "
					+ "fieldGetter=null, " + "fieldSetter=null]] " + "shadows field with the same name in "
					...
					+ "fieldGetter=null," + " fieldSetter=null]]]", ae.getMessage());
		}
	}
	...
	public void preConfiguredShadowFieldsSkipped() {
		PojoClass aChildWithFieldShadowing = PojoClassFactory.getPojoClass(NoShadowAChildWithFieldShadowing.class);
		validator = ValidatorBuilder.create().with(new NoFieldShadowingRule("aField")).build();
		validator.validate(aChildWithFieldShadowing);
	}
	...
	public void nonMatchingSkipFieldsHaveNoEffect() {
		PojoClass aChildWithFieldShadowing = PojoClassFactory.getPojoClass(NoShadowAChildWithFieldShadowing.class);
		validator = ValidatorBuilder.create().with(new NoFieldShadowingRule("a non existing field name")).build();
		try {
			validator.validate(aChildWithFieldShadowing);
			Assertions.fail("Should've failed shadow check");
