...
public class NoStaticExceptFinalRuleTest {
	...
	public void shouldIgnoreSyntheticFields() throws ClassNotFoundException {
		...
		final String className = this.getClass().getPackage().getName() + ".AClassWithStaticSyntheticField"
		...
		CommonCode.shouldPassRuleValidation(rule,
				simpleClassLoader.loadThisClass(AClassWithSyntheticStaticFieldDumper.dump(classNameAsPath), className));
	}
