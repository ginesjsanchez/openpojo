...
public class DefaultValidatorTest {
	...
	public void rulesAndTestersAreTriggeredWhenValidation() {
		...
		Assertions.assertEquals(1, validatedPojoClasses.size());
		Assertions.assertEquals(validatedPojoClasses.get(0), PojoClassFactory.getPojoClass(DummyClass.class));
	...
	public void rulesAndTestersAreTriggeredWhenValidationIsRunRecursively() {
		...
		Assertions.assertEquals(2, validatedPojoClasses.size());
		Assertions.assertTrue(validatedPojoClasses.contains(PojoClassFactory.getPojoClass(DummyClass.class)));
		Assertions.assertTrue(validatedPojoClasses.contains(PojoClassFactory.getPojoClass(AnotherDummyClass.class)));
	...
	private void assertInvokedClasses(List<String> invocations, String... classNames) {
		Assertions.assertEquals(classNames.length, invocations.size());

		for (String className : classNames)
			Assertions.assertTrue(invocations.contains(className));
	}
