...
public class LoggerFactoryTest {
	...
	public final void shouldReturnDefaultLoggerClassByClass() {
		...
		Assertions.assertEquals(defaultLoggerClass.getName(), log.getClass().getName());
	}
	...
	public final void shouldReturnSetLogger() throws ClassNotFoundException {
		for (String logger : supportedLoggers) {
			...
			Affirm.affirmTrue(
					String.format("Expected LoggerFactory to be set to [%s] but was [%s]", loggerClass,
							LoggerFactory.getLogger((String) null)),
					LoggerFactory.getLogger((String) null).getClass().equals(loggerClass));
		}
	}
}
