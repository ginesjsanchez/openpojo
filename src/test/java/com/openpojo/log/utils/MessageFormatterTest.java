...
public class MessageFormatterTest {
	...
	private List<UsingCurlyBracketsTestData> getUsingCurlyBracketsTestData() {
		...
		usingCurlyBracketsTestData
				.add(new UsingCurlyBracketsTestData(null, null, new Object[] { "nothing to format to" }));
		...
		usingCurlyBracketsTestData.add(
				new UsingCurlyBracketsTestData("Two Params a string=[myString] and a date=[" + date.toString() + "]",
		...
		for (int idx = 0; idx < 3; idx++) {
			...
			fakeStackTraceElements[idx] = (StackTraceElement) InstanceFactory.getInstance(
					PojoClassFactory.getPojoClass(StackTraceElement.class), RandomFactory.getRandomValue(String.class),
					RandomFactory.getRandomValue(String.class), RandomFactory.getRandomValue(String.class), lineNumber);
		...
		usingCurlyBracketsTestData
				.add(new UsingCurlyBracketsTestData(expectedLog, "Exception=[{0}]", new Object[] { exception }));
		...
		usingCurlyBracketsTestData
				.add(new UsingCurlyBracketsTestData("Message and one param [param one] and this one is null=[null]",
						"Message and one param [{0}] and this one is null=[{1}]", new Object[] { "param one", null }));

		usingCurlyBracketsTestData.add(new UsingCurlyBracketsTestData(
				"Nested Array unfolds [[1, 2, 3], [check, me, out]]", "Nested Array unfolds [{0}, {1}]",
	...
	private List<FlattenArrayToStringTestData> getFlattenArrayToStringTestData() {
		...
		flattenArrayToStringTestData
				.add(new FlattenArrayToStringTestData("[[1, 2, 3]]", new Object[] { new Integer[] { 1, 2, 3 } }));
	...
	public final void testUsingCurlyBrackets() {
		for (final UsingCurlyBracketsTestData entry : getUsingCurlyBracketsTestData()) {
			Assertions.assertEquals(entry.expected, MessageFormatter.usingCurlyBrackets(entry.message, entry.fields));
		}
	}
	...
	public final void testFlattenArrayToString() {
		for (final FlattenArrayToStringTestData entry : getFlattenArrayToStringTestData()) {
			Assertions.assertEquals(entry.expected, Arrays.toString(MessageFormatter.formatArgsToStrings(entry.array)));
		}
	...
	public final void testBoundaryConditionsOnGenerateCurlyBracketTokens() {
		...
		Assertions.assertEquals("", MessageFormatter.generateCurlyBracketTokens(Integer.MIN_VALUE));
		Assertions.assertEquals("", MessageFormatter.generateCurlyBracketTokens(-1));
		Assertions.assertEquals("", MessageFormatter.generateCurlyBracketTokens(0));
		Assertions.assertEquals(
				GENERATE_CURLY_BRACKET_TOKEN_PREFIX + TOKEN_COUNTER_START + GENERATE_CURLY_BRACKET_TOKEN_POSTFIX,
	...
	public final void testActualArraySentAsObject() {
		...
		Assertions.assertEquals(expected, MessageFormatter.format(array));
	}
	...
	public void formatArrayOfArrays() {
		...
		Assertions.assertEquals(expected, MessageFormatter.format(arrayOfArray));
	}
	...
	public final void testRandomGenerateCurlyBracketTokens() {
		...
		Assertions.assertEquals(assertString,
