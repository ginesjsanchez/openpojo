...
public class BusinessIdentityTest {
	...
	public void testIncompleteObject() {
		try {
			...
			Assertions.fail("Expected Exception due to required BusinessKeys not fullfilled");
		...
		try {
			...
			Assertions.fail("Expected Exception due to composite BusinessKeys not fullfilled");
		} catch (final BusinessException be) {
			// expected
		}
		...
		try {
			...
			Assertions.fail("Expected Exception due to no BusinessKeys defined");
		} catch (final BusinessException be) {
			// expected
		}
		...
		try {
			...
			Assertions.fail("Expected Exception due to null object");
		} catch (final BusinessException be) {
			// expected
		}
	}
	...
	public void testToString() {
		...
		Assertions.assertTrue(
				...
				toString.startsWith("com.openpojo.business.BusinessIdentityTest$ToStringTestData [@")
						&& toString.endsWith(
								": instance_name=Instance Name, static_name=Static Name, STATIC_FINAL_NAME=Static Final Name]"));
	}
	...
	public void shouldToStringOnArrays() {
		...
		expected += "]";
		Assertions.assertEquals(expected, toStringOutput);
	}
	...
	public void whenNullObject_Then_toString_returnNull() {
		Assertions.assertEquals("null", BusinessIdentity.toString(null));
	}
	...
	private static class HashCodeTestData {
		...
		public HashCodeTestData(final String id, final String name, final String optionalPart1,
				final String optionalPart2, final String nonBusinessKey) {
			this.id = id;
			this.name = name;
