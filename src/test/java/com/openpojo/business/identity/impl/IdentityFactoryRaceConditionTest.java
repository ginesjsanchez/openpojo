...
public class IdentityFactoryRaceConditionTest {
	...
	public void runWithManyThreads() {
		...
		Assertions.assertEquals(numberOfThreads, tpe.getCompletedSuccessfully());
	}

	private ReportingThreadExecutionPool getReportingThreadExecutionPool(int numberOfThreads) {
		return new ReportingThreadExecutionPool(numberOfThreads, numberOfThreads, 3 * 60, TimeUnit.SECONDS,
				new ArrayBlockingQueue<Runnable>(numberOfThreads));
	}

	private Validator getValidator() {
		return ValidatorBuilder.create().with(new BusinessKeyMustExistRule()).with(new GetterMustExistRule())
				.with(new NoFieldShadowingRule()).with(new NoNestedClassRule()).with(new NoPrimitivesRule())
				.with(new NoPublicFieldsExceptStaticFinalRule()).with(new NoPublicFieldsRule())
				.with(new NoStaticExceptFinalRule()).with(new SerializableMustHaveSerialVersionUIDRule())
				.with(new SetterMustExistRule()).with(new BusinessIdentityTester()).with(new DefaultValuesNullTester())
