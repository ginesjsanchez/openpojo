				...
				if (pojoMethod.getReturnType().isAssignableFrom(field.getType())) {
					if (pojoMethod.isAbstract()) {
						LoggerFactory.getLogger(
								PojoMethodFactory.class)
								.warn("Getter=[{}] in class=[{}] rejected due to method being abstract",
					...
					LoggerFactory.getLogger(
							PojoMethodFactory.class)
							.warn("Getter=[{}] in class=[{}] rejected due non-equal return types [{} != {}]",
									pojoMethod.getName(), field.getDeclaringClass().getName(),
		...
		for (final String candidateName : generateSetMethodNames(field)) {
			...
			if (pojoMethod != null) {
				if (pojoMethod.isAbstract()) {
					LoggerFactory.getLogger(
							PojoMethodFactory.class)
							.warn("Setter=[{}] in class=[{}] rejected due to method being abstract",
									pojoMethod.getName(), field.getDeclaringClass().getName());
					pojoMethod = null;
				}
				break;
