...
public final class ValidationHelper {
	...
	/**
	 * Applies the rules first and the testers afterwards to a class, skipping the cases that cannot be instantiated.
	 *
	 * @param pojoClass
	...
	public static void runValidation(PojoClass pojoClass, List<Rule> rules, List<Tester> testers) {
		...
		if (pojoClass.isSynthetic()) {
			logger.warn(
					"Attempt to validate synthetic class=[{}] ignored, consider using FilterSyntheticClasses filter when "
							+
							"calling PojoClassFactory",
					pojoClass.getClazz());
			return;
		}
		...
		if ((pojoClass.isInterface() || pojoClass.isEnum()) && testers.size() > 0) {
			logger.warn("Attempt to execute behavioural test on non-constructable class=[{}] ignored",
					pojoClass.getClazz());
			return;
		}
		...
		try {
			...
			logger.warn(
					"ASM not loaded while attempting to execute behavioural tests on non-constructable class[{}], either "
							+
							"filter " + "abstract classes or add asm to your classpath.",
					pojoClass.getClazz());
		}
	}
