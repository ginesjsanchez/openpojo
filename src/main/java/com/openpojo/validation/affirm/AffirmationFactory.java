...
public final class AffirmationFactory {
	/**
	 * Assertion back ends, in order of preference. The last one throws a plain
	 * {@code AssertionError}, which every test framework reports as a failure, so it works
	 * even when none is on the classpath.
	 */
	...
	private AffirmationFactory() {
		...
		log.info("Dynamically setting affirmation implementation = [{}]", affirmation);
	}
