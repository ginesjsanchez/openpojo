...
public class NoFieldShadowingRule implements Rule {
	...
	/**
	 * Creates the rule leaving the given fields out.
	 *
	 * @param fieldNamesToSkip
	 *     Field names that are not checked.
	 */
	...
	public void evaluate(final PojoClass pojoClass) {
		...
		for (final PojoField childPojoField : childPojoFields) {
			if (!childPojoField.isSynthetic() && !isSerializable(childPojoField, pojoClass)
					&& !inSkipList(childPojoField))
				if (contains(childPojoField.getName(), parentPojoFields))
					Affirm.fail("Field=[" + childPojoField + "] shadows field with the same name in parent class=["
							+ parentPojoFields + "]");
		}
