...
public class BusinessIdentityUtils {
	 ...
	 */
	public static boolean areEqual(final BusinessKeyField pojoField, final Object first, final Object second,
		...
		if (first.getClass().isArray()) {
			...
			for (int idx = 0; idx < Array.getLength(first); idx++) {
				runningEquality = runningEquality
						&& doEquals(Array.get(first, idx), Array.get(second, idx), caseSensitive);
			}
	 ...
	 */
	public static int getHashCode(final BusinessKeyField pojoField, final Object instance,
			final boolean caseSensitive) {
